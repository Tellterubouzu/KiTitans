...
# from torch import nn, Tensor
# from torch.utils.data import DataLoader, IterableDataset
# from transformers import AutoTokenizer, get_cosine_schedule_with_warmup  # トークナイザーとスケジューラー用
...
#     WANDB_AVAILABLE = True
#     PROJECT_NAME = 'titans-mac-transformer-0.3B'
#     RUN_NAME = f'mac - {16} longterm mems, layers {(2,4,6,8,10,12,14)}, warmup+Cosine'
#     WANDB_ONLINE = True
#     wandb.init(project=PROJECT_NAME, mode='disabled' if not WANDB_ONLINE else 'online')
#     wandb.run.name = RUN_NAME
#     wandb.run.save()
...
# # ─────────────────────────────
# # ⑧ DeepSpeed による初期化と分散並列の設定、学習率スケジューラーの実装
# # ─────────────────────────────
# def main():
#     parser = argparse.ArgumentParser()
#     parser.add_argument("--local_rank", type=int, default=0)
...
#     # ここでウォームアップ + コサイン減衰スケジューラーを設定
#     # 例として、全体の5%をウォームアップに充てる（NUM_BATCHES の5%）
#     warmup_steps = int(NUM_BATCHES * 0.05)
...
# ─────────────────────────────
# ⑧ DeepSpeed による初期化と分散並列の設定、スクラッチ実装したスケジューラーの適用
# ─────────────────────────────
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--local_rank", type=int, default=0)
    ...
    # スクラッチ実装したウォームアップ＋コサイン減衰スケジューラーの初期化
    warmup_steps = int(NUM_BATCHES * 0.05)  # 例: 総ステップの5%をウォームアップ
    scheduler = CosineWarmupScheduler(optimizer, warmup_steps, NUM_BATCHES, LEARNING_RATE)
